...
class NICO_SUB(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        all_class = ['bear', 'cow', 'dog', 'horse', 'rat', 'bicycle', 'bus', 'car',
                     'helicopter', 'motorcycle', 'train', 'truck', 'elephant',
                     'monkey', 'sheep', 'airplane', 'bird', 'boat', 'cat']
        ...
        train_x = self.generate_fewshot_dataset_balance(train, num_shots=num_shots)
        val = self.generate_fewshot_dataset(val, num_shots=num_shots)
        train_samples = self.generate_fewshot_dataset_balance(train, num_shots=num_shots)
        
        print('all domain names:', all_domain)

        super().__init__(train_x=train_x, val=val, test=test, train_samples=train_samples, alldomain=all_domain)
