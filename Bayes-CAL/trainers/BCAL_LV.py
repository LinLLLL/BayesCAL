...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, sigma=1e-5):
        ...
        ctx_vectors = torch.empty(n_cls, 1024)
    ...
    def update_init(self, image_features, labels, labels_t, image_features_pre, text_features_pre):
        # update_init_for_fs_learning
        ctx_vectors = torch.empty(self.n_cls, 1024)
        nn.init.normal_(ctx_vectors, std=0.02)
        for idx, name in enumerate(self.classnames):
            for i in range(image_features.shape[0]):
                if idx == labels_t[i].item():
                    image_feature = image_features[i].repeat(image_features_pre.shape[0], 1)
                    image_feature = (image_feature / (image_feature.norm(dim=-1, keepdim=True) + 1e-6))
                    image_features_pre = (image_features_pre / (image_features_pre.norm(dim=-1, keepdim=True) + 1e-6))

                    # ctx_vectors initialized by Clustering Centres of pre-training data
                    similarity = image_feature @ image_features_pre.t()
                    min_idx = torch.argmax(similarity[0]).item()

                    ctx_vectors[idx] = text_features_pre[labels[min_idx]]
        self.ctx_init = ctx_vectors.detach()
        self.ctx_sigma_init = self.sigma * torch.ones_like(ctx_vectors)

        # Mean vectors
        self.ctx = nn.Parameter(ctx_vectors)  # to be optimized
        # Sigma vectors
        self.ctx_sigma = nn.Parameter(self.sigma * torch.ones_like(ctx_vectors))  # to be optimized
