...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, sigma=1e-5):
        ...
        n_ctx = cfg.TRAINER.CAL.N_CTX
        ctx_init = cfg.TRAINER.CAL.CTX_INIT
...
#            ctx_vectors += 10
        ...
        self.class_token_position = cfg.TRAINER.CAL.CLASS_TOKEN_POSITION
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        image_features_context = (self.image_encoder_context_branch(image_features) / (self.image_encoder_context_branch(image_features).norm(dim=-1, keepdim=True) +1e-6)).type(self.dtype)
...
class Logits(nn.Module):
    ...
    def forward(self, image_features, text_features_category, text_features_context):
        logit_scale = self.logit_scale.exp()
        logits_category = logit_scale * (image_features / (image_features.norm(dim=-1, keepdim=True)+1e-6)).type(text_features_category.dtype) @ text_features_category.t()
        logits_context = logit_scale * (self.image_encoder_context_branch(image_features) / (1e-6+self.image_encoder_context_branch(image_features).norm(dim=-1, keepdim=True))).type(text_features_context.dtype) @ text_features_context.t()
...
class CAL(TrainerX):
    """Context Optimization (CAL).
    """
   
    def check_cfg(self, cfg):
        assert cfg.TRAINER.CAL.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.CAL.PREC == "fp32" or cfg.TRAINER.CAL.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.CAL.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        # print(image.shape)
        prec = self.cfg.TRAINER.CAL.PREC
        if prec == "amp":
            with autocast():
